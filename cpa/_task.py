...
class CPATrainingPlan(TrainingPlan):
    ...
    #     if hasattr(self.module.pert_network, "kinetics_head"):
    #         ae_params += list(filter(lambda p: p.requires_grad, self.module.pert_network.kinetics_head.parameters()))
    #         ae_params += list(filter(lambda p: p.requires_grad, self.module.pert_network.target_embedding.parameters()))
    #         ae_params += list(filter(lambda p: p.requires_grad, self.module.pert_network.direction_head.parameters()))
    ...
    def configure_optimizers(self):
        ...
        if hasattr(self.module.pert_network, "kinetics_head"):
            ...
            target_mode = getattr(self.module.pert_network, "_target_mode", "random")
            if target_mode == "frozen_projector":
                ae_params += list(filter(lambda p: p.requires_grad,
                                        self.module.pert_network.target_feature_norm.parameters()))
                ae_params += list(filter(lambda p: p.requires_grad,
                                        self.module.pert_network.target_projector.parameters()))
            else:
                ae_params += list(filter(lambda p: p.requires_grad,
                                        self.module.pert_network.target_embedding.parameters()))
    ...
    def on_train_epoch_end(self):
        # Access logged metrics instead of outputs directly
        try:
            # For metrics that are logged automatically, we can access them via self.logger
            for key in self.metrics:
                if key in ['disnt_basal', 'disnt_after']:
                    self.epoch_history[key].append(0.0)
                else:
                    # Use a default value if metric is not available
                    if hasattr(self, 'logger') and hasattr(self.logger, 'metrics'):
                        val = self.logger.metrics.get(f'train/{key}', 0.0)
                        self.epoch_history[key].append(val)
                    else:
                        self.epoch_history[key].append(0.0)
            ...
            if self.current_epoch > 1 and self.step_size_lr is not None and self.current_epoch % self.step_size_lr == 0:
                try:
    ...
    def on_validation_epoch_end(self, outputs=None):
        for key in self.metrics:
            if outputs and any(key in out for out in outputs):
        ...
        self.log('val_recon', self.epoch_history['recon_loss'][-1] if 'recon_loss' in self.epoch_history else 0.0, prog_bar=True)
        self.log('cpa_metric', self.epoch_history['cpa_metric'][-1] if 'cpa_metric' in self.epoch_history else 0.0, prog_bar=False)
        self.log('disnt_basal', self.epoch_history['disnt_basal'][-1] if 'disnt_basal' in self.epoch_history else 0.0, prog_bar=True)
        self.log('disnt_after', self.epoch_history['disnt_after'][-1] if 'disnt_after' in self.epoch_history else 0.0, prog_bar=True)
        self.log('val_r2_mean', self.epoch_history['r2_mean'][-1] if 'r2_mean' in self.epoch_history else 0.0, prog_bar=True)
        self.log('val_r2_var', self.epoch_history['r2_var'][-1] if 'r2_var' in self.epoch_history else 0.0, prog_bar=False)
        self.log('val_KL', self.epoch_history['KL'][-1] if 'KL' in self.epoch_history else 0.0, prog_bar=True)
