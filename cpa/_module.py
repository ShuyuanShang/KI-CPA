...
from ._utils import PerturbationNetwork, VanillaEncoder, CPA_REGISTRY_KEYS, KineticsPerturbationNetwork
...
def build_drug_to_target_mapping(adata, pert_encoder: dict, control_group: str,
    ...
    Returns
    -------
    drug_to_target : torch.LongTensor, shape (n_perts,)
        与 pert_encoder 的索引顺序对齐；<PAD> 和 control_group 指向 dummy target (index 0)
    n_targets : int
        含 dummy 在内的 target 总数
    target_name_to_idx : dict
        真实 target 名 -> target index 的映射（idx从1开始，0留给dummy），方便后续可解释性分析
    """
    obs = adata.obs
    drug_to_target_name = (
        obs.loc[obs["pert_iname"] != control_group, ["pert_iname", target_col]]
        .drop_duplicates()
        .set_index("pert_iname")[target_col]
        .to_dict()
    )

    # 断言：确认确实是单靶点（每个药物只对应一个target），否则下面的映射会静默出错
    check = obs.loc[obs["pert_iname"] != control_group].groupby("pert_iname", observed=True)[target_col].nunique()
    assert (check <= 1).all(), (
        f"以下药物对应了多个 TTD Target ID，与单靶点假设不符，请检查数据："
        f"{check[check > 1].index.tolist()}"
    )

    unique_targets = sorted(set(drug_to_target_name.values()))
    target_name_to_idx = {t: i + 1 for i, t in enumerate(unique_targets)}  # 0 留给 dummy
    ...
    if missing:
        print(f"警告: 以下 {len(missing)} 个药物在obs里找不到对应的 {target_col}，"
              f"已指向dummy target(0)，请人工核查: {missing[:10]}{'...' if len(missing) > 10 else ''}")

    return drug_to_target, n_targets, target_name_to_idx


# ------------------------------------------------------------------
# 使用示例（伪代码，字段名换成你实际的列名）
# ------------------------------------------------------------------
if __name__ == "__main__":
    import anndata

    adata = anndata.read_h5ad("/root/autodl-tmp/data/single_target_drug_GSE70138_processed.h5ad")

    # 注意：必须用 KineticsCPA.setup_anndata(...) 而不是 cpa.CPA.setup_anndata(...)。
    # scvi-tools 会把"调用setup_anndata时的类名"记进registry，后面 KineticsCPA.load()
    # 会校验这个类名对不对得上，用错类调用会导致load()直接报错。
    KineticsCPA.setup_anndata(
        ...
        smiles_key="SMILES",       # 触发rdkit Morgan FP计算
        max_comb_len=1,
    )
    ...
    print(f"n_targets(含dummy) = {n_targets}, missing按上面警告信息核查")
    ...
    model = KineticsCPA(
        ...
        n_latent=256,
        recon_loss="gauss",
        doser_type="logsigm",     # 这是CPA原生路径自己的doser类型，kinetics分支不受此参数影响
    )

    model.train(max_epochs=200, use_gpu=True)

    # 训练过程中监控kinetics分支是否坍缩（对应设计文档4.3节）：
    # batch = next(iter(some_dataloader))
    # print(model.module.pert_network.diagnostics(batch['perts'], batch['perts_doses']))